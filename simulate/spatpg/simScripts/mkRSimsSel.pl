...
for($r=0; $r<$nrep; $r++){
    ...
    print R "write.table(round(rnorm(100),5),\"rsimSelE"."$r".".txt\",row.names=F,quote=F,col.names=F)\n";
    print R "write.table(G,\"rsimSelG"."$r".".txt\",row.names=F,col.names=F,quote=F)\n";
    print R "write.table(round(X,4),\"rsimSelP"."$r".".txt\",row.names=F,col.names=F,quote=F)\n";
    close(R);
