...
for($r=0; $r<$nrep; $r++){
    ...
    print R "x<-k + (j-1) * $ng\n";
    print R "w<-rep(NA,3)\n";
    print R "w[1]<-linearCurve(A=0.99,B=0.01,evar[x])\n";
    print R "w[2]<-linearCurve(A=0.99,B=0,evar[x])\n";
    print R "w[3]<-linearCurve(A=0.99,B=-0.01,evar[x])\n";
    ...
    print R "write.table(round(evar,5),\"rsimWeakFluctLE"."$r".".txt\",row.names=F,quote=F,col.names=F)\n";
    print R "write.table(G,\"rsimWeakFluctLG"."$r".".txt\",row.names=F,col.names=F,quote=F)\n";
    print R "write.table(round(X,4),\"rsimWeakFluctLP"."$r".".txt\",row.names=F,col.names=F,quote=F)\n";
    close(R);
    system "R CMD BATCH source.R\n";

## convert allele frequency file from R to spatgen

    $in = "rsimWeakFluctLG"."$r".".txt";
